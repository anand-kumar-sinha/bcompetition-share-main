...
//        user_advertise_share_history
        ...
        if(isset($auth_id) && isset($auth_token)){
            ...
            if($result > 0){ 
                ...
                //check
                $modelDatass['tableName'] = "student_transfer_money";
                // $modelDatass['condtion']  = "student_id=" . $auth_id . " AND status = Pending";
                // $UserDetailss = $this->SystemModel->getOne($modelDatass);
                $UserDetailss = $this->SystemModel->getRequrestData($auth_id);
                $is_request = "No";
                if(!empty($UserDetailss)){
                    $is_request = "Yes";
                }
                //end
    ...
    */
        ...
        $result = $this->SystemModel->tableRowCount($modelData);
         ...
         if ($result > 0) {
            ...
            $getLeve1StatusInfo = $this->SystemModel->getLeve1StatusInfo($auth_id);
              if($amount <= $getLeve1StatusInfo->wallet_amount){
            ...
            $modelDatass['tableName'] = "store";
                $modelDatass['condtion']  = "id=" . $course_id . "";
                $UserDetailss = $this->SystemModel->getOne($modelDatass);
                ...
                $finalStudentAMount = $studentinfodata->wallet_amount - $amount;
              ...
              $student_wallet_historyData['tableName'] = "student_wallet_history";   
                $student_wallet_historyData['data'] = array(     
                                                ...
                                                'transaction_number'    => $UserDetailss->course_title.' - Courses',
    ...
    //course deatils api
        ...
        if(isset($category_id) && isset($auth_id) && isset($auth_token)){
            ...
            $store_list['condtion'] = "course_status = 1 AND course_category_id=" . $category_id;
            ...
            if ($result > 0) {
                ...
                foreach ($StoreList as $Pkey => $Pvalue) {
                    ...
                    $student_sub['condtion'] = "status= 1 AND student_id=" . $auth_id. " AND course_id = ".$Pvalue->id ;
                    $StudentSub = $this->SystemModel->tableRowCount($student_sub);
                    $isSubscription = 0;
                    if($StudentSub > 0){
                        $isSubscription = 1;
                    }
                    $expire_date = date('Y-m-d h:i:s', strtotime($Pvalue->created. ' + '.intval($Pvalue->course_validity).' months'));
                    $totalExpir = intval($Pvalue->course_validity) + 1;
                     $totalExpirDate = date('Y-m-d h:i:s', strtotime($Pvalue->created. ' + '.intval($totalExpir).' months'));

                     if($expire_date >= $totalExpirDate){
                        $isSubscription = 0;
                     }
                    $StoreList[$Pkey]->expire_date= $expire_date;
    ...
    public function get_subscription_list(){
        ...
        if(isset($category_id) && isset($auth_id) && isset($auth_token)){
            ...
            // $subscription_list['condtion'] = "status= 1 AND student_id=" . $auth_id. " AND course_id = ".$category_id;
            // $ResultNoti = $this->SystemModel->tableRowCount($subscription_list);
             $subscription_list['tableName'] = "student_subscription_details";        
            $subscription_list['condtion'] = "status= 1 AND student_id=" . $auth_id;
            $ResultNoti = $this->SystemModel->tableRowCount($subscription_list);
            ...
            if ($ResultNoti > 0) {
                $SubscriptionList = $this->SystemModel->getAll($subscription_list);
                 ...
                 foreach ($SubscriptionList as $Pkey => $Pvalue) {
                    ...
                    $course_id = $Pvalue->course_id;
                    $getstoreinfo = $this->SystemModel->getstoreinfo($course_id);
                    if(!empty($getstoreinfo)){
                        if($getstoreinfo->course_category_id != $category_id){
                            unset($SubscriptionList[$Pkey]);
                        }
                    }
                 ...
                 foreach ($SubscriptionList as $Pkey => $Pvalue) {
                    ...
                    $totalExpir = intval($Pvalue->course_validity) + 1;
                     ...
                     if($expire_date >= $totalExpirDate){
                            unset($StoreList[$Pkey]);
                     }else{
                         $StoreList[$Pkey]->expire_date= $expire_date;
                    $StoreList[$Pkey]->course_image = base_url('uploads/store_image/'. $Pvalue->course_image);
                    $StoreList[$Pkey]->is_subscribed= 0;
                     }
     ...
     public function getLiveClassChat(){
       //  print_r($this->input->get());exit;
        ...
        if(isset($liveclass_chat_id ) && isset($auth_id) && isset($auth_token)){
           ...
           //                              ');
           $ChatList = $this->SystemModel->get_join_meeting_details_chat_api($liveclass_chat_id, $auth_id);
            if (!empty($ChatList)) {
                            ...
                            if(!empty($subdata)){
                                ...
                                foreach ($subdata as $Skey => $Svalue) {
                                    if(!empty($Svalue)){
                                         ...
                                         $totalAmount = $this->SystemModel->getmanage_autopool_levelRow($Svalue->package_id);
                                         $amount = $totalAmount->package_price;
           ...
           foreach ($testactivedata as $Pkey => $Pvalue) {
                ...
                $levelamount = $this->SystemModel->getLevel($levelid);
                 ...
                 if(!empty($level1)){
                    foreach ($level1 as $Lkey => $Lvalue) {
                        if($Lvalue != 0){
                            $subdatas = $this->SystemModel->student_attempt_test($Lvalue);
                           $subdata = $this->SystemModel->student_test_subscription($Lvalue);
                            if(!empty($subdata)){
                                    $totaltestsubmit1 = $totaltestsubmit1 + count($subdatas);
                                foreach ($subdata as $Skey => $Svalue) {
                                    if(!empty($Svalue)){
                                          
                                        // $amount = $this->SystemModel->getstoreinfo($Svalue->category_id);
                                          $amount = $Svalue->join_amount;
                                        $totalTestSubmitAmount1 = $totalTestSubmitAmount1 + $amount;
               ...
               //  echo $totalTestActiveAmount1;
                 ...
                 if(!empty($level2)){
                    foreach ($level2 as $Lkey => $Lvalue) {
                        if($Lvalue != 0){
                           ...
                           $subdata = $this->SystemModel->student_test_subscription($Lvalue);
                            if(!empty($subdata)){
                                    $totaltestsubmit2 = $totaltestsubmit2 + count($subdatas);
                                foreach ($subdata as $Skey => $Svalue) {
                                    if(!empty($Svalue)){
                                          
                                        //   $amount = $this->SystemModel->getstoreinfo($Svalue->category_id);
                                          $amount = $Svalue->join_amount;
                                        $totalTestSubmitAmount2 = $totalTestSubmitAmount2 + $amount;
                 ...
                 if(!empty($level3)){
                    foreach ($level3 as $Lkey => $Lvalue) {
                        if($Lvalue != 0){
                           ...
                           $subdata = $this->SystemModel->student_test_subscription($Lvalue);
                            if(!empty($subdata)){
                                    $totaltestsubmit3 = $totaltestsubmit3 + count($subdatas);
                                foreach ($subdata as $Skey => $Svalue) {
                                    if(!empty($Svalue)){
                                          
                                        //  $amount = $this->SystemModel->getstoreinfo($Svalue->category_id);
                                          $amount = $Svalue->join_amount;
                                        $totalTestSubmitAmount3 = $totalTestSubmitAmount3 + $amount;
                 ...
                 if(!empty($level4)){
                    foreach ($level4 as $Lkey => $Lvalue) {
                        if($Lvalue != 0){
                           ...
                           $subdata = $this->SystemModel->student_test_subscription($Lvalue);
                            if(!empty($subdata)){
                                    $totaltestsubmit4 = $totaltestsubmit4 + count($subdatas);
                                foreach ($subdata as $Skey => $Svalue) {
                                    if(!empty($Svalue)){
                                          
                                         //  $amount = $this->SystemModel->getstoreinfo($Svalue->category_id);
                                          $amount = $Svalue->join_amount;
                                        $totalTestSubmitAmount4 = $totalTestSubmitAmount4 + $amount;
                ...
                $levelamount = $this->SystemModel->getLevel($levelid);
                    ...
                    foreach ($level5 as $Lkey => $Lvalue) {
                        if($Lvalue != 0){
                            $subdatas = $this->SystemModel->student_attempt_test($Lvalue);
                             $subdata = $this->SystemModel->student_test_subscription($Lvalue);
                            if(!empty($subdata)){
                                    $totaltestsubmit5 = $totaltestsubmit5 + count($subdatas);
                                foreach ($subdata as $Skey => $Svalue) {
                                    if(!empty($Svalue)){
                                          
                                         // $amount = $this->SystemModel->getstoreinfo($Svalue->category_id);
                                         $amount = $Svalue->join_amount;
                                        $totalTestSubmitAmount5 = $totalTestSubmitAmount5 + $amount;
                 ...
                 $totalTestSubmitAmount1 = (float)$totalTestSubmitAmount1 * (float)$levelamount1 / 100;
           ...
           foreach ($testactivedata as $Pkey => $Pvalue) {
               ...
               //  echo $totalTestActiveAmount1;
                ...
                $levelamount = $this->SystemModel->getLevel($levelid);
                 ...
                 $totalTestActiveAmount1 = (float)$totalTestActiveAmount1 * (float)$levelamount1 / 100;
                            ...
                            if(!empty($subdata)){
                                ...
                                foreach ($subdata as $Skey => $Svalue) {
                                    if(!empty($Svalue)){
                                        ...
                                        $totalStudentCourseAmount1 = $totalStudentCourseAmount1 + $amount->course_offer_price;
                            ...
                            if(!empty($subdata)){
                                ...
                                foreach ($subdata as $Skey => $Svalue) {
                                    if(!empty($Svalue)){
                                        ...
                                        $amount = $this->SystemModel->getstoreinfo($Svalue->course_id);
                                        $totalStudentCourseAmount2 = $totalStudentCourseAmount2 + $amount->course_offer_price;
                            ...
                            if(!empty($subdata)){
                                ...
                                foreach ($subdata as $Skey => $Svalue) {
                                    if(!empty($Svalue)){
                                        ...
                                        $amount = $this->SystemModel->getstoreinfo($Svalue->course_id);
                                        $totalStudentCourseAmount3 = $totalStudentCourseAmount3 + $amount->course_offer_price;
                            ...
                            if(!empty($subdata)){
                                ...
                                foreach ($subdata as $Skey => $Svalue) {
                                    if(!empty($Svalue)){
                                        ...
                                        $amount = $this->SystemModel->getstoreinfo($Svalue->course_id);
                                        $totalStudentCourseAmount4 = $totalStudentCourseAmount4 + $amount->course_offer_price;
                            ...
                            if(!empty($subdata)){
                                ...
                                foreach ($subdata as $Skey => $Svalue) {
                                    if(!empty($Svalue)){
                                        ...
                                        $amount = $this->SystemModel->getstoreinfo($Svalue->course_id);
                                        $totalStudentCourseAmount5 = $totalStudentCourseAmount5 + $amount->course_offer_price;
...
$level5TotalAmounts = number_format((float)$level5TotalAmount, 2, '.', '');    
    ...
    public function saveAutoPoolPackage()
    {
        ...
        if(isset($auth_id) && isset($auth_token)){
            ...
            if ($result > 0) {
                   ...
                   $getStudentLastData = $this->SystemModel->getStudentLastData($package_id);
            ...
            $totalCOunts = $getStudentLastData->new_seq + 1;
                    ...
                    if($payment_mode == "online"){
                        ...
                        //echo $getLeve1StatusInfo->wallet_amount;exit;
                        if($amount <= $getLeve1StatusInfo->wallet_amount){
                            ...
                            $this->SystemModel->savePackagePoolData($data);
                                ...
                                $modelDatass['tableName'] = "manage_autopool_level";
                $modelDatass['condtion']  = "id=" . $package_id . "";
                $UserDetailss = $this->SystemModel->getOne($modelDatass);
                ...
                $student_wallet_historyData['data'] = array(     
                                                'student_id'    => $auth_id,
                                                'wallet_amount'    => $amount,
                                                'transaction_number'    => $UserDetailss->package_name.' - Auto Pool',
                                                'old_wallet_amount' => $finalStudentAMount,
                                                'transaction_type'    => "Debit",
                                                'status'    => "Completed",
                                                 'package_id' => $package_id,
    ...
    public function packageDetails()
    {
        ...
        if(isset($auth_id) && isset($auth_token)){
            ...
            $getAutoPoolDataInfo = $this->SystemModel->getAutoPoolData($auth_id, $package_id);
            ...
            if ($result > 0) {
                ...
                if(!empty($student_complete_group_levels)){
                    $level1Satus = "Pending";
                    $level1Direct = $getmanage_autopool_levelRow->level1_group_member * $getAutoPoolDataInfo->new_seq;
                    ...
                    // }
                    $level2Satus = "Pending";
                    $level2Direct = $getmanage_autopool_levelRow->level2_group_member  * $getAutoPoolDataInfo->new_seq;
                    ...
                    // }
                    $level3Satus = "Pending";
                    $level3Direct = $getmanage_autopool_levelRow->level3_group_member  * $getAutoPoolDataInfo->new_seq;
                    ...
                    // }
                    $level4Satus = "Pending";
                    $level4Direct = $getmanage_autopool_levelRow->level4_group_member  * $getAutoPoolDataInfo->new_seq;
                    ...
                    // }
                    $level5Satus = "Pending";
                    $level5Direct = $getmanage_autopool_levelRow->level5_group_member  * $getAutoPoolDataInfo->new_seq;
                    ...
                    // }
                    $direstStudent = count($getAutoPoolData);


                    $getstudent_refer = $this->SystemModel->getstudent_refer_info_table($auth_id);
                  //  echo '<pre>';print_r($getstudent_refer);exit;
                    $$totalPurCout= 0;
                    if(!empty($getstudent_refer)){
                        foreach ($getstudent_refer as $key => $value) {
                            //echo $package_id;exit;
                            $getPurchaseInfo = $this->SystemModel->getPurchaseInfostudent_buy_autopool_package($value->student_id, $package_id);
                            if(!empty($getPurchaseInfo)){
                                $totalPurCout = $totalPurCout + 1;
                            }
                        }
                    }
                    ...
                    if($totalPurCout >= 2){ 
                        $totalBuyHoss = 0;
                    }else if($totalPurCout <= 2 && !empty($totalPurCout)){ 
                        $totalBuyHoss = $totalPurCout;
                    }else{
                        $totalBuyHoss = 2;
                    }
 //echo $totalBuyHoss;exit;
     $level1amount = 0;
                    if($level1Satus == "Completed"){
                        $level1amount = $student_complete_group_levels->student_first_group_member_amount;
                    }
                    $level2amount = 0;
                    if($level2Satus == "Completed"){
                        $level2amount = $student_complete_group_levels->student_second_group_member_amount;
                    }
                    $level3amount = 0;
                    if($level3Satus == "Completed"){
                        $level3amount = $student_complete_group_levels->student_third_group_member_amount;
                    }
                    $level4amount = 0;
                    if($level4Satus == "Completed"){
                        $level4amount = $student_complete_group_levels->student_fourth_group_member_amount;
                    }
                    $level5amount = 0;
                    if($level5Satus == "Completed"){
                        $level5amount = $student_complete_group_levels->student_fifth_group_member_amount;
                    }
                    // $packageInfo = [
                    //     'student_id' => $auth_id,
                    ...
                    $packageInfo = [
                        ...
                        'level4_after_id_income' => $level4Direct,
                        'level4_status' => $level4Satus,
                        'level4_amount' => $level4amount,
                        'level5_after_id_income' => $level5Direct,
                        'level5_status' => $level5Satus,
                        'level5_amount' => $level5amount,
                        'total_income' => $level1amount + $level2amount + $level3amount + $level4amount + $level5amount,
                        'direct_total' => $totalBuyHoss
                    ];


                    $student_group_level = $this->SystemModel->student_group_level($package_id);
                    //data add into database
                   $getmanage_autopool_levelRowInfo = $this->SystemModel->getmanage_autopool_levelRow($package_id);
                   
                   
                    if($totalBuyHoss == 0){
                        if($level1Satus != "Pending"){
                            $getLeve1Status = $this->SystemModel->getLeve1Status($auth_id, $package_id);
                            $getLeve1StatusInfo = $this->SystemModel->getLeve1StatusInfo($auth_id);

                            if($getLeve1Status->auto_pool_status == 1){
                               



                            }else{
                                $egtmanddg_package_info = $this->SystemModel->egtmanddg_pmanage_autopool_level($package_id);

                                $total3per =  $student_group_level->level1_group_member_amount; 
                                $totalRefer1 = $total3per;

                                 $totalWallet = $getLeve1StatusInfo->wallet_amount + $totalRefer1;
                                            $student_wallet_historyData1['tableName'] = "student_wallet_history";   
                                            $student_wallet_historyData1['data'] = array(     
                                                                            'student_id'    => $auth_id,
                                                                            'wallet_amount'    => $totalRefer1,
                                                                            'old_wallet_amount' => $totalWallet,
                                                                            'transaction_number'    => "Auto Pool Amount",
                                                                            'transaction_type'    => "Credit",
                                                                            'status'    => "Completed",
                                                                            'package_id' => $package_id,
                                                                            'created'  => date('Y-m-d h:i:s'),
                                            ...
                                            $result = $this->SystemModel->insertData($student_wallet_historyData1);
                        ...
                        if($level2Satus != "Pending"){
                            $getLeve2Status = $this->SystemModel->getLeve2Status($auth_id, $package_id);
                            $getLeve1StatusInfo = $this->SystemModel->getLeve1StatusInfo($auth_id);
                            if($getLeve2Status->auto_pool_status == 2){
                                ...
                                $egtmanddg_package_info = $this->SystemModel->egtmanddg_pmanage_autopool_level($package_id);
                                $total3per = $student_group_level->level2_group_member_amount; 
                                $totalRefer2 = $total3per;
                                 $totalWallet = $getLeve1StatusInfo->wallet_amount + $totalRefer2;
                                            $student_wallet_historyData1['tableName'] = "student_wallet_history";   
                                            $student_wallet_historyData1['data'] = array(     
                                                                            'student_id'    => $auth_id,
                                                                            'wallet_amount'    => $totalRefer2,
                                                                            'old_wallet_amount' => $totalWallet,
                                                                            'transaction_number'    => "Auto Pool Amount",
                                                                            'transaction_type'    => "Credit",
                                                                            'status'    => "Completed",
                         ...
                         if($level3Satus != "Pending"){
                            $getLeve3Status = $this->SystemModel->getLeve3Status($auth_id, $package_id);
                            $getLeve1StatusInfo = $this->SystemModel->getLeve1StatusInfo($auth_id);
                            if($getLeve3Status->auto_pool_status == 3){
                                ...
                                $totalRefer3 = $total3per;

                                 $totalWallet = $getLeve1StatusInfo->wallet_amount + $totalRefer3;
                                            $student_wallet_historyData1['tableName'] = "student_wallet_history";   
                                            $student_wallet_historyData1['data'] = array(     
                                                                            'student_id'    => $auth_id,
                                                                            'wallet_amount'    => $totalRefer3,
                                                                            'old_wallet_amount' => $totalWallet,
                                                                            'transaction_number'    => "Auto Pool Amount",
                                                                            'transaction_type'    => "Credit",
                                                                            'status'    => "Completed",
                         ...
                         if($level4Satus != "Pending"){
                            $getLeve4Status = $this->SystemModel->getLeve4Status($auth_id, $package_id);
                           // echo '<pre>';print_r($getLeve4Status);exit;
                            $getLeve1StatusInfo = $this->SystemModel->getLeve1StatusInfo($auth_id);
                            if($getLeve4Status->auto_pool_status == 4){
                                 ...
                                 $egtmanddg_package_info = $this->SystemModel->egtmanddg_pmanage_autopool_level($package_id);
                                $total3per =  $student_group_level->level4_group_member_amount; 
                                $totalRefer4 = $total3per;
                                  $totalWallet = $getLeve1StatusInfo->wallet_amount + $totalRefer4;
                                            $student_wallet_historyData1['tableName'] = "student_wallet_history";   
                                            $student_wallet_historyData1['data'] = array(     
                                                                            'student_id'    => $auth_id,
                                                                            'wallet_amount'    => $totalRefer4,
                                                                            'old_wallet_amount' => $totalWallet,
                                                                            'transaction_number'    => "Auto Pool Amount",
                                                                            'transaction_type'    => "Credit",
                                                                            'status'    => "Completed",
                         ...
                         if($level5Satus != "Pending"){
                            $getLeve5Status = $this->SystemModel->getLeve5Status($auth_id, $package_id);
                            $getLeve1StatusInfo = $this->SystemModel->getLeve1StatusInfo($auth_id);
                            if($getLeve5Status->auto_pool_status == 5){
                                 ...
                                 $egtmanddg_package_info = $this->SystemModel->egtmanddg_pmanage_autopool_level($package_id);
                                $total3per =  $student_group_level->level5_group_member_amount; 
                                $totalRefer5 = $total3per;
                                 $totalWallet = $getLeve1StatusInfo->wallet_amount + $totalRefer5;
                                            $student_wallet_historyData1['tableName'] = "student_wallet_history";   
                                            $student_wallet_historyData1['data'] = array(     
                                                                            'student_id'    => $auth_id,
                                                                            'wallet_amount'    => $totalRefer5,
                                                                            'old_wallet_amount' => $totalWallet,
                                                                            'transaction_number'    => "Auto Pool Amount",
                                                                            'transaction_type'    => "Credit",
                                                                            'status'    => "Completed",
                  ...
                  //  echo '<pre>';print_r($getAutoPoolData);exit;
                    $direstStudent = 2;
                    $student_complete_group_levels = $this->SystemModel->student_complete_group_levels($auth_id, $package_id);
                    $getmanage_autopool_levelRow = $this->SystemModel->getmanage_autopool_levelRow($package_id);
                     $packageInfo = [
                        'student_id' => $auth_id,
                        'level1_after_id_income' => $getmanage_autopool_levelRow->level1_group_member * $auth_id,
                        'level1_status' => "Pending",
                        'level1_amount' => 0,
                        'level2_after_id_income' => $getmanage_autopool_levelRow->level2_group_member * $auth_id,
                        'level2_status' => "Pending",
                        'level2_amount' => 0,
                        'level3_after_id_income' => $getmanage_autopool_levelRow->level3_group_member * $auth_id,
                        'level3_status' => "Pending",
                        'level3_amount' => 0,
                        'level4_after_id_income' => $getmanage_autopool_levelRow->level4_group_member * $auth_id,
                        'level4_status' => "Pending",
                        'level4_amount' => 0,
                        'level5_after_id_income' => $getmanage_autopool_levelRow->level5_group_member * $auth_id,
         ...
         if(isset($auth_id) && isset($auth_token)){
            ...
            $result = $this->SystemModel->tableRowCount($modelData);
                if ($result > 0) {
                    ...
                    $package_list['condtion'] = "status = 1" ;
                     ...
                     foreach ($packgeList as $key => $value) {
                        if(!empty($value->package_thumbnil)){
                         ...
                         $packgeList[$key]->package_thumbnil = "https://jbcompetitionclasses.com/uploads/package_image/".$value->package_thumbnil;
                        }
                        if(!empty($value->package_image)){

                         $packgeList[$key]->package_image = "https://jbcompetitionclasses.com/uploads/package_image/".$value->package_image;
                        }
               ...
               //  echo '<pre>';print_r($packgeList);exit;
                 foreach ($packgeList as $key => $value) {
                    ...
                    if(!empty($getmanage_autopool_levelRow->package_thumbnil)){
                     ...
                     $packgeList[$key]->package_thumbnil = "https://jbcompetitionclasses.com/uploads/package_image/".$getmanage_autopool_levelRow->package_thumbnil;
                    }
                    if(!empty($getmanage_autopool_levelRow->package_image)){

                     $packgeList[$key]->package_image = "https://jbcompetitionclasses.com/uploads/package_image/".$getmanage_autopool_levelRow->package_image;
                    }
         ...
         if ($result > 0) {
                      ...
                      foreach ($PackageList as $key => $value) {
                        ...
                        $PackageList[$key]->is_purchasable = $checkStatus;
                          $PackageList[$key]->package_thumbnil = "https://jbcompetitionclasses.com/uploads/package_image/".$value->package_thumbnil;
                          $imageArr = explode(",",$value->package_image);
                            foreach ($imageArr as $Ikey => $Ivalue) {
                                  $imageArrs[] = [
                                    'imagename' => $value->image_name,
                                    'image'   => "https://jbcompetitionclasses.com/uploads/package_image/".$Ivalue                                         
                                  ];
                            ...
                            foreach ($linkArr as $Ikey => $Ivalue) {
                                  //$linkArrs[]['link'] = $Ivalue;
                                   $linkArrs[] = [
                                    ...
                                    'link'   => "https://jbcompetitionclasses.com/uploads/package_image/".$Ivalue                                         
                                  ];
                              }  
            ...
            if($payment_mode == "online"){
                ...
                if($amount <= $getLeve1StatusInfo->wallet_amount){
                   $student_subscription_historyData['tableName'] = "package_purchase_info"; 
                    ...
                    $modelDatass['tableName'] = "package_info";
                $modelDatass['condtion']  = "id=" . $package_id . "";
                $UserDetailss = $this->SystemModel->getOne($modelDatass);
                ...
                $student_wallet_historyData['data'] = array(     
                                                'student_id'    => $auth_id,
                                                'wallet_amount'    => $amount,
                                                'transaction_number'    => $UserDetailss->package_name.' - Package',
                                                'old_wallet_amount' => $finalStudentAMount,
                                                'transaction_type'    => "Debit",
                                                'status'    => "Completed",
                                                'created'  => date('Y-m-d h:i:s'),
         ...
         // echo '<pre>';print_r($getData);exit;
          //add first level cont
            ...
            if(count($getData) >= $student_group_level->level1_group_member){
                ...
                //$total1per = ($egtmanddg_package_info->package_price * $student_group_level->level1_group_member_amount) / 100; 
